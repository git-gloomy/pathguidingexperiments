...
namespace GuidedPathTracerExperiments.Integrators {
    ...
    public class AdamGuidedPathTracer : PathTracer {
        ...
        protected AdamProbabilityTree probabilityTree;
        // Utility used to keep track of all data needed to learn guiding probabilities during path generation
        protected ThreadLocal<AdamPathSegmentStorage> adamPathSegmentStorage = new();
        ...
        int debugVisualizationInterval = 32;
        ...
        protected override void OnStartPath(PathState state) {
            ...
            if (!adamPathSegmentStorage.IsValueCreated)
                adamPathSegmentStorage.Value = new();

            pathSegmentStorage.Value.Reserve((uint)MaxDepth + 1);
            pathSegmentStorage.Value.Clear();

            adamPathSegmentStorage.Value.Clear();
        }
        ...
        protected override (Ray, float, RgbColor) SampleDirection(in Ray ray, in SurfacePoint hit, PathState state) {
            ...
            var adamSegment = adamPathSegmentStorage.Value.LastSegment;
            Ray nextRay;
            ...
            if (state.Rng.NextFloat() < selectGuideProb) { // sample guided
                ...
                adamSegment.UseForLearning = true;
            ...
            if (pdf == 0) { // prevent NaNs / Infs
                adamSegment.UseForLearning = false;
                return (new(), 0, RgbColor.Black);
            }
            ...
            adamSegment.SamplePdf = pdf;
            adamSegment.ScatteringWeight = contrib;
            adamSegment.BsdfCosine = hit.Material.EvaluateWithCosine(hit, outDir, inDir, false);
        ...
        protected override void OnNextEventResult(in Ray ray, in SurfacePoint point, PathState state,
            ...
            var adamSegment = adamPathSegmentStorage.Value.LastSegment;
            adamSegment.ScatteredContribution += contrib;
        }
        ...
        protected override void OnHitLightResult(in Ray ray, PathState state, float misWeight, RgbColor emission,
            ...
            if (isBackground) {
                ...
                var newAdamSegment = adamPathSegmentStorage.Value.NextSegment();
                newAdamSegment.Position = ray.Origin + ray.Direction * scene.Radius * 1337;
            }
        ...
        protected override void OnFinishedPath(in RadianceEstimate estimate, PathState state) {
            ...
            adamPathSegmentStorage.Value.EvaluatePath(probabilityTree);
        }
    }
}